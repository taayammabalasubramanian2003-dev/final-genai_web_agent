...
if not api_key:
    st.error("❌ Critical Error: API Keys missing. Please set OPENAI_API_KEY and PINECONE_API_KEY in Streamlit Secrets.")
    st.stop()
...
class MemoryAgent:
    ...
    def __init__(self, api_key):
        ...
        # Check/Create Index (1024 dims for compatibility)
...
class ConversationalAgent:
    ...
    def chat(self, user_input, history, image_base64=None, system_context=""):
        ...
        system_msg = f"""
        You are FinBot, an expert financial analyst. 
        Current Context: {system_context}
        
        Analyze questions and charts. Be helpful, concise, and professional.
        """
...
# Rename key to avoid collision with form
...
# --- PROFILE ---
    ...
    with st.form("profile_form_ui"):
        ...
        if st.form_submit_button("Save Profile"):
            # Update the specific data dictionary, not the whole state object
...
# --- AI DECISION ---
    ...
    if st.session_state.stock_analysis_data:
        ...
        if st.button("Ask Advisor for Detail"):
            # Uses Chat Node but with specific context
...
# --- FINANCIAL PLANNING ---
    ...
    st.metric("Expected Wealth", f"₹{future:,.0f}")
...
st.sidebar.divider()
st.sidebar.caption("System: Multi-Agent RAG + Vision v3.1")
